...
module.exports.verify = function verify(email, options, callback) {
  ...
  if (!validator(params.email)) return params.callback(null, {
    success: false,
...
function beginSMTPQueries(params) {
  ...
  socket.once('end', function () {
    ...
    callback(null, {
      ...
      addr: params.email,
      code: infoCodes.finishedVerification,
      tryagain: tryagain,
      banner: banner
    })
  })
...
module.exports.verifyAsync = email => new Promise((resolve, reject) => {
  this.verify(email, (err, info) => err ? reject(err) : resolve(info));
});
